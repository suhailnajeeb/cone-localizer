...
# World Settings
h_camera = 340 # mm
alpha_camera = 20 # degrees

# Path to nn Blob File
nnBlobPath = str((Path(__file__).parent / Path('../models/yolov8n_det_3510_yb_6shave.blob')).resolve().absolute())

# Make sure the blob file exists
if not Path(nnBlobPath).exists():
    import sys
    raise FileNotFoundError(f'Required file/s not found, please run "{sys.executable} install_requirements.py"')

# Cone Detection Label Map
...
# Object to transform Camera Frame into World Frame
cam2world = CameraToWorld(h_camera, alpha_camera)
...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        for detection in detections:
            ...
            x_w, y_w, z_w = cam2world.transform_to_body_frame(x_c, y_c, z_c)
